...
This does well with our given conditions but doesn't tell us the total number of apples that were consumed on days where there was no soccer practice. In order to crunch that number, we'll need to introduce a new function called `append()`. `append()` literally appends values to the end of an existing vector. For example...
